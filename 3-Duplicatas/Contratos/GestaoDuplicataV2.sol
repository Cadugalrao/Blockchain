contract GestaoDuplicata
{
    ...
    function consultar_duplicata(uint _chave) constant returns (uint, uint, uint, uint64, uint8, string, uint) {
        ...
        /*
        Alocacao alocacao_aux = alocacoes[msg.sender];

        bool associacao_ativa = false;
        bool associacao_encontrada = false;
        for (uint a = alocacoes[msg.sender].chave.length; a > 0 ; a--) {
            if (alocacao_aux.chave[a - 1] == _chave) {
                associacao_ativa = alocacao_aux.ativa[a - 1];
                associacao_encontrada = true;
                break;
            }
        }
        */
        ...
        /*
        if (associacao_encontrada && !associacao_ativa)
            situacao_aux = 0;
        */
        ...
        //if (duplicata_aux.instituicao_que_cadastrou == msg.sender)
