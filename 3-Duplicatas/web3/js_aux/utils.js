	...
	/*var projects = [
		{
			...
			value: "jquery",
			label: "Sizzle JS",
			desc: "R$ 7.999.999.999,99"
		},
		{
			index: 2,
			value: "jquery-ui",
			label: "Sizzle JS",
			desc: "R$ 8.999.999.999,99"
		},
		{
			index: 3,
			value: "sizzlejs",
			label: "Sizzle JS",
			desc: "R$ 9.999.999.999,99"
		}
	];
	*/
...
function retorna_descricao_tipo(tipo) {
	for (var b = 0;b < config[2].Tipo_duplicatas.length; b++)
		if (config[2].Tipo_duplicatas[b].codigo == tipo)
			return config[2].Tipo_duplicatas[b].descricao;
}
...
function inicializar_contrato() {
	...
	$( "#incluir_duplicata_btn" ).click(function() {
		...
		if (valor_duplicata.trim().length == 0) {
			alert("O valor informado &eacute; inv&aacute;lido")
			return;
		}
		*/

		//if (valor_duplicata.indexOf(",") == -1)
		//	valor_duplicata = valor_duplicata + ",00";
		var chave_duplicata_big = new BigNumber(chave_duplicata, 10);
		...
		//for (var i = 0; i < 60; i++) {
		//	aux_valor = parseInt(valor_duplicata) + i
		//	tipo_duplicata = Math.floor((Math.random() * 5) + 0);
	...
	//event ret_alterar_situacao_duplicata(uint128 chave, uint8 nova_situacao, int cod_erro, string descricao);
    /*
	gestao_duplicatas.ret_alterar_situacao_duplicata(function(error, result){
		if (error != null) {
			registrarEvento("Altera&ccedil;&atilde;o da Situa&ccedil;&atilde;o da Duplicata", result.transactionHash, false, result, error, "", 0);
		} else {
			var cod_erro = parseInt(result.args.cod_erro);
			registrarEvento("Altera&ccedil;&atilde;o da Situa&ccedil;&atilde;o da Duplicata", result.transactionHash, ((cod_erro < 0) ? true : false), result, error, "", cod_erro);
		}
	});
	*/
