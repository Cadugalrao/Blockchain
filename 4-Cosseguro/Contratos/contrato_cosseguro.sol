contract Cosseguro
{
    ...
    function incluir_apolice(string _codigo, uint64 _premio, uint64 _cobertura, uint _dt_vencimento) {
        ...
        if (equal(_codigo, "")) {
            ret_incluir_apolice(_codigo, _premio, _cobertura, _dt_vencimento, 0, 0, 0, 0, false, -1, "Codigo inválido");
        } else if (_premio == 0) {
            ret_incluir_apolice(_codigo, _premio, _cobertura, _dt_vencimento, 0, 0, 0, 0, false, -1, "Premio inválido");
        } else if (_cobertura == 0) {
            ret_incluir_apolice(_codigo, _premio, _cobertura, _dt_vencimento, 0, 0, 0, 0, false, -1, "Cobertura inválida");
        } else if (_dt_vencimento < block.timestamp) {
            ret_incluir_apolice(_codigo, _premio, _cobertura, _dt_vencimento, 0, 0, 0, 0, false, -1, "Data de vencimento inválida");
        } else {
    ...
    */
    ...
    function incluir_acordo(uint _index_apolice, uint16 _percent_acordo, address _addr_seguradora_aceito) {
        if (_index_apolice >= _qtdeApolices) {
            ret_incluir_acordo(_index_apolice, _percent_acordo, _addr_seguradora_aceito, 0, false, -1, "Index da apolice invalido");
        } else {
            Apolice memory apolice_aux = apolices[_index_apolice];

            if (apolice_aux.addr_seguradora[0] != msg.sender) {
                ret_incluir_acordo(_index_apolice, _percent_acordo, _addr_seguradora_aceito, 0, false, -1, "Instituicao solicitante nao eh responsavel pela apolice");
            } else if (_percent_acordo > 0) {
                ret_incluir_acordo(_index_apolice, _percent_acordo, _addr_seguradora_aceito, 0, false, -1, "Percentual invalido");
            } else {
                uint qtde_seguradoras = contar_seguradoras(_index_apolice);

                address _addr_seguradora_apolice;
                uint _index_acordo_apolice;
                uint16 _acordo_percent_aux = _percent_acordo;
                for (uint iAcordos = 0; iAcordos < qtde_seguradoras; iAcordos++) {
                ...
                if (_acordo_percent_aux >= 100) {
                    ret_incluir_acordo(_index_apolice, _percent_acordo, _addr_seguradora_aceito, 0, false, -1, "Percentual invalido. Ultrapassa a soma de 100.");
                } else {
    ...
    function autorizar_acordo(uint _index_apolice_cedido, uint _index_acordo_aceito, address _addr_cedido) {
        if (_index_apolice_cedido >= _qtdeApolices) {
            ret_autorizar_acordo(_index_apolice_cedido, _index_acordo_aceito, _addr_cedido, false, -1, "Index da apolice invalido");
        } else {
            ...
            if (addr_cedido_aux != _addr_cedido) {
                ret_autorizar_acordo(_index_apolice_cedido, _index_acordo_aceito, 0, false, -1, "Index da apolice invalido");
